...
( function( $ ) {
	...
	$.fn.heplbox = function( settings ) { //crée la fonction
		...
		var defaultSettings = {
				...
				//image directement encodée dans le plugin
				cssPrefix: "hepl-box",
				overlayOpacity:0.66,
				zIndexBase:9999
			},
			settings = $.extend(defaultSettings, settings), //permet de récupérer les valeurs passées dans settings tout en complétant avec les valeurs de defaultSettings
			elements = [], //on récupère les liens et on les met dans un tableau
			currentIndex = 0,
			$overlay,
			$container;
		...
		var elementClicked = function(e){
			...
			currentIndex = elements.indexOf($(this).attr("href")); //retrouver le bon url dans le tableau
		...
		var openBox = function(){
			...
			$container.fadeIn("normal", function(){ loadImage(); } );
		...
		$(this).on("click.heplbox", elementClicked); // Espace de nom = espace virtuel où toutes les variables sont préfixées par un nom. On ne réagit pas à un clic, mais à un click.heplbox ==> sécurité
		
		});
	};

} )( jQuery ); //encapsulation >> $ = l'objet JQuery (par sécurité)